...
WidgetLED		WidgetFan(V8);			//Inicjacja diody LED dla załączania wentylatora
...
const char	auth[]			= "GkXIcT6IWvb0w6RTJyVWsQUe5u5bipe3";	// Token Łazienka Rymanowska
...
void Bathrum_Humidity_Control()
{
	if (Tryb_Sterownika == 2)		//Wilgotność w trybie ręcznym OFF
	{
		...
		WidgetFan.off();					//Wentylator Wyłączony
	}
	else if (Tryb_Sterownika == 1 && HeatCO == false)			//Wilgotność w trybie ręcznym ON
	{
		...
		WidgetFan.on();					//Wentylator włączony
	}
...
BLYNK_WRITE(V11)
{
	switch (param.asInt())
	{
		case 1:				//AUTO
			...
			break;
		case 2:				//ON
			if ( HeatCO == 0 )		// Sprawdza czy piec grzeje 0 --> piec nie grzeje, 1 --> piec grzeje
			{
				Tryb_Sterownika = 1;
				Blynk.setProperty(V10,"color","#163C49"); //Zdławiony-niebieski
				break;
			...
			{
				Blynk.notify("Nie można włączyć wentylatora kiedy piec grzeje!");
				Blynk.virtualWrite(V11, Tryb_Sterownika + 1);	// Zmienia wartość przycisków na taką jaka był poprzednio
			}	
		case 3:				//OFF
			Tryb_Sterownika = 2;
